...
function showQuestion() {
  ...
  var voiceText = quizArray[currentQuestion].question + " the choices are: ";
  ...
  for (i = 0; i < quizArray[currentQuestion].options.length; i++) {
	...
	questionBtn.setAttribute(
      "id",
      "select-"+quizArray[currentQuestion].options[i].replace(/\s/g, '-').toLowerCase()
    );
    questionBtn.setAttribute(
      "class",
	...
	voiceText += quizArray[currentQuestion].options[i];
  }
...
function disableQuestions() {
  ...
  questionsAssed.forEach((element) => {
    element.setAttribute(
      "class",
      "list-group-item list-group-item-action btn-danger mt-1 answerButton disabled"
    );
    if (
      ...
      element.setAttribute(
        "class",
        "list-group-item list-group-item-action btn-success mt-1 answerButton disabled"
      );
    }
  });
}
